...
| 8 | `SCI-004` | Feature, not remediation; Q5 adopted; M1/M2 accepted; M3 remains rejected after the future-R3 unit gate stopped | XL | D29 design correction now; later five-path R3 and S3 remain separately gated (WP-9) |
...
- **E4 — `SCI-004` last.** `Tier7JonesSciencePlan.md` §18.3 already enumerates
  ...
  discipline still governs every new m-mode fingerprint. Those original
  dependencies are satisfied, but correction #29 now requires D29 followed by
  a complete-unit-green five-path validator/oracle/evidence-binding R3 before
  replacement S3 may resume.
...
| WP-9 | `SCI-004` design gate and phase-separated successor | XL | M1/M2 accepted; M3 requires D29, then a separately accepted five-path R3 directly over D29 before replacement S3 |
...
Correction #29 records the stopped D28-based future-R3 unit gate and separates
the current evidence-envelope design from the immutable D24 and D25 designs
inside the correction-#24 and correction-#25 red records. D29 is design-only.
The next R3 directly parents exact D29 and may change only the five paths ruled
in the design memo; it must make the complete serial unit suite green and
directly test the historical/current split before review. Replacement S3 then
directly parents that R3 under a separately authorized, hash-authenticated
clean-worktree reconstruction. No retained record, production path, phase
artifact, or status changes in D29.
...
- **Q5 (WP-9 science driver):** *resolved 2026-08-11 by program adoption* — a
  ...
  gate; it does not accept a production phase or close `SCI-004`. Correction
  #29 leaves that driver unchanged and only rules the D29-to-R3
  historical/current evidence-design binding and succession.
...
| WP-9 | Design gate ACCEPTED 2026-08-21 with the complete correction chain recorded in `docs/development/sci004_mmode_design.md`; M1 ACCEPTED 2026-08-23 and M2 ACCEPTED 2026-08-24 remain immutable. Correction #24 landed as `4d507bf1333ccaa4c8beec3815370ba0f6043bb2`, followed by fresh R3 `944e0ee66ebdaffafab86f4f8f4253a404aa902c`, replacement S3 `b07925ab14b56b3ca0fa863f806290748a31df6b`, candidate E3 `886e62fd9f8328826b388b8960ed7413da26b6d1`, and canonical A3 REJECT `8529da951e2378115ffde8d5da3e2af56f3323d0`. The rejected attempt's retained characterization input digest lacked a reconstructible path-independent preimage (`m3.fingerprint-input-preimage-not-retained`); M3 was not accepted, no successor or closure was unlocked, and SCI-004 remains ROADMAP. Bounded correction #25 rules a v2 retained path-independent characterization manifest, immutable rejected-attempt authentication, exact fixed-path disposal/null-sentinel restoration, approved-parent first-parent selection for repeated E3/A3 introductions, a fresh fingerprint post-source red delta, and replacement S3/E3/A3 authorities. Fresh independent physics/governance and computational reviewers both returned `ACCEPT` on the identical round-3 pinned candidate bytes and complete diff after two bounded fix rounds; only the design header's final verification sentence and this ledger verdict completion postdate those pins. Correction #25 is design-only: no implementation, evidence regeneration, M3 acceptance, or SCI-004 closure is recorded, and SCI-004 remains ROADMAP. Original fingerprint R3 `a65c53a46e84f63c163c5ad15fba8645df33d1d2` then retained the official two-red/three-green observation, but the uncommitted replacement-S3 governance run stopped at its mandatory detached-replay gate because the shared editable Pixi interpreter selected the primary checkout's dirty production source and returned five passes. Bounded correction #26 classifies only that deterministic validator replay-venue provenance defect, preserves the oracle, supplement, scientific partition, and exact nine-path replacement-S3 authority, and authorizes a fresh two-validator-path R3 with a closed detached-`src` environment and fail-closed import-provenance preflight. Fresh independent physics/governance reviewer `/root/c26_physics_governance_review` and computational/provenance reviewer `/root/c26_computational_provenance_review` each returned exact `ACCEPT` on identical round-1 pinned correction-#26 candidate bytes and complete diff; only the design header's final verification sentence and this ledger verdict completion postdate those pins. Correction #26 is design-only. Correction #26 `93321d331e4f6442d39fe79588be6f05ad4bee42` and fresh validator-only R3 `929263f3376e472e0b6da53c6c4e093c10ba7465` are published; that old R3 and its historically correct D26 dependency binding remain immutable. The replacement-S3 continuation stopped at the complete-unit Tier-8 citation gate. Bounded correction #27 repairs only the governed correction-#26 prose/citation defect. Fresh independent physics/governance reviewer `/root/c27_physics_governance_review` and computational/provenance reviewer `/root/c27_computational_provenance_review` each returned exact `ACCEPT` on identical round-3 pinned candidate bytes and the complete diff after two bounded fix rounds; only the design header's final review-verification sentence and this ledger verdict completion postdate those pins. D27 `82fb0773890870a6fb90b3ed9b8065df89919a84` remains immutable. An earlier D27-based replacement-S3 attempt then stopped before edits when its authority preflight reviewer returned `REJECT`: protected unmarked node `tests/characterization/test_sci004_mmode.py::test_a_family_pin_is_a_ci001_observation_set_not_a_bare_digest` still requires singleton `radiosim.sci004.characterization-input.v1`, while replacement S3 must publish `radiosim.sci004.characterization-input.v2`, producing the exact observed v1/v2 assertion mismatch. The primary nine-path S3 bytes remain preserved, uncommitted, and unmodified. Bounded correction #28 is design-only: once locally landed it makes D27 an immutable superseded design and becomes the operative design binding, but it is not published or accepted as a production phase. It preserves mandatory production v2 and defines, but does not create, a later single-parent non-merge R3 directly over D28 with exactly three modified paths — `tests/characterization/test_sci004_mmode.py`, `tests/unit/test_sci004_phase3_dependency.py`, and `tests/unit/test_sci004_phase3_red_failures.py` — to implement the exact v1-at-R3/v2-at-S3 transition oracle, freeze D28, authenticate historical red provenance at original fingerprint R3, and retain the two-red/three-green partition and all three red records. Eventual S3 directly parents that new R3 and keeps exactly five modified paths — `src/radiosim/core/result.py`, `tools/sci004_mmode_phase3_evidence.py`, `tests/unit/test_sci004_phase3_evidence.py`, `tools/sci004_mmode_phase3_acceptance.py`, and `tests/unit/test_sci004_phase3_acceptance.py` — plus four deletions — `docs/development/sci004_mmode_phase3_evidence.json`, `docs/development/sci004_mmode_phase3_evidence.md`, `output/benchmarks/reference/sci004/20260825T122048Z-macbook-pro-2.json`, and `docs/development/sci004_mmode_phase3_acceptance.json`; no fourth R3 path or tenth S3 path is authorized. Future S3 uses `design_sha = D28`, `red_commit_sha = future post-D28 R3`, and all six approved bindings remain literal `None`. D28 does not publish or accept R3/S3/E3/A3. Replacement S3 remains pending and uncommitted; M3 remains rejected/not accepted; SCI-004 remains ROADMAP; E3 and A3 remain unavailable; closure `C` remains locked. PERF-001 remains ROADMAP, and no accepted accelerator-performance record exists. No accelerator, GPU, public diffuse/HEALPix, non-scalar-beam, public end-to-end backend-wiring, speedup, general-performance, PERF-001 acceptance, production-readiness, phase-acceptance, or closure claim is licensed. The D28-based three-path R3 attempt then passed gates 1--8 but stopped fail-closed at the complete serial unit suite with exact result `3 failed, 7435 passed, 2 skipped, 30 warnings in 597.61s`; all three failures came from the evidence tool equating correction-#24's immutable D24 record design with current `_design_sha()`. That candidate remains unreviewed, unstaged, uncommitted, and preserved. Bounded correction #29 is design-only: after D29 lands, D28 is an immutable superseded design and a future five-path validator/oracle/evidence-binding R3 directly parents D29. The governed minimum is correction #28's three paths plus `tools/sci004_mmode_phase3_evidence.py` and `tests/unit/test_sci004_phase3_evidence.py`, separating the D24/D25 historical record bindings from the D29 current-envelope binding and directly rejecting mutation or current-design substitution. The future R3 must restart every gate and make the complete serial unit suite green before review. A later clean-worktree S3 reconstruction composes both overlapping evidence paths from the exact R3 parent blobs, hash-authenticated primary S3 intended bytes, and the D29 invariant; the same five-modification/four-deletion inventory survives only after both evidence paths are proved to retain S3-owned deltas. No historical JSON is rewritten, and the 29 + 6 + 2 red inventory and exact two-red/three-green partition remain immutable. M3 remains rejected/not accepted; SCI-004 and PERF-001 remain ROADMAP; E3 and A3 remain unavailable; closure `C` remains locked; and no accelerator, GPU, public diffuse/HEALPix, non-scalar-beam, public end-to-end backend-wiring, speedup, general-performance, production-readiness, phase-acceptance, or closure claim is licensed. Fresh independent correction-#29 governance/authority and evidence/provenance reviews are pending against identical candidate pins. |
