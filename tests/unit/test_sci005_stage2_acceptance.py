"""Strict authentication of the SCI-005 Stage-2 acceptance record.

``docs/development/sci005_beam_physics_plan.md`` Section 7.5 freezes this
module's successor authority: it lands in ``S2`` with both approved constants
as the literal ``None``, the official acceptance path absent, and every
synthetic strict schema/digest/ancestry fixture passing. ``A2`` then changes
...
whichever file happens to be checked out, Section 8.3 adds the corrected
``U1 ->* D2`` succession the Stage-2 tool authenticates from Git objects alone,
and Section 9 freezes the read-only verifier's certificate, its key order, and
its six stderr prefixes.
...
sci005_stage2_acceptance.schema.json`` remains the normative transcription of
Section 8.2; :func:`validate_stage2_acceptance` enforces the same structure,
...
#: Section 7.5's two approved constants. ``A2`` replaces exactly these two
...
ARTIFACT = "docs/development/sci005_stage2_acceptance.json"
SCHEMA = "docs/development/sci005_stage2_acceptance.schema.json"
...
#: Section 8.2's exact Stage-2 oracle identifiers.
REQUIRED_ORACLES = (
...
#: Section 8.3's observed ``U1..D2`` interval, transcribed from the memo header.
INTERVAL_KINDS: dict[str, str] = {
    "3e336095009e72bf4ae6064d5e97d381e063258f": "status-prose",
    "3d60b6f428e9ac94407f50dfd9153aad27d5e098": "superseded-design",
    "0c378153f5d5fa46574ca334d109b40e102f12e6": "superseded-design",
    "2a5d5aa7fe84a464368993a96c395fc444c2cedb": "superseded-red-slice",
}
...
def _tool() -> Any:
    ...
    try:
        import sci005_stage2_acceptance as module
    finally:
        sys.path.pop(0)
    return module
...
def validate_stage2_acceptance(document: Any) -> None:
    """Authenticate one Stage-2 acceptance document against Section 8.2."""
    root = _mapping(document, "$", ACCEPTANCE_KEYS)
    if root["schema_version"] != "radiosim.sci005.stage2-acceptance.v1":
        _fail("$.schema_version", "is not the Stage-2 acceptance literal")
    ...
    if root["evidence_artifact_path"] != EVIDENCE_ARTIFACT:
        _fail("$.evidence_artifact_path", "is not the frozen Stage-2 path")
    if root["evidence_schema_path"] != EVIDENCE_SCHEMA:
        _fail("$.evidence_schema_path", "is not the frozen Stage-2 path")
    ...
    if toolchain["acceptance_generator_path"] != TOOL:
        _fail("$.toolchain.acceptance_generator_path", "is not the Stage-2 tool")
    ...
    if toolchain["acceptance_schema_path"] != SCHEMA:
        _fail("$.toolchain.acceptance_schema_path", "is not the Stage-2 schema")

    if root["acceptance_commit_sha"] is not None:
        _fail("$.acceptance_commit_sha", "is JSON null; U2 binds the containing A2")
...
def _synthetic_acceptance() -> dict[str, Any]:
    ...
    return {
        ...
        "implementation_identity": "sci005-s2-implementer",
        "reviewer_identity": "sci005-s2-independent-reviewer",
...
def test_the_null_sentinels_and_the_absent_artifact_agree() -> None:
    """At ``S2`` both constants are ``None`` and the artifact is absent."""
    ...
    validate_stage2_acceptance(document)
...
def test_this_validator_loads_only_the_standard_library() -> None:
    ...
    # ``sci005_stage2_acceptance`` is the tool under test, itself stdlib-only.
    assert roots <= {
        "__future__",
        "hashlib",
        "json",
...
def test_the_acceptance_tool_also_loads_only_the_standard_library() -> None:
    completed = subprocess.run(
        [
            ...
            "import sci005_stage2_acceptance; "
...
def test_the_schema_transcription_and_the_validator_agree() -> None:
    ...
    assert schema["properties"]["schema_version"]["const"] == (
        "radiosim.sci005.stage2-acceptance.v1"
    )
...
def test_a_complete_synthetic_acceptance_document_validates() -> None:
    validate_stage2_acceptance(_synthetic_acceptance())
...
def test_a_reordered_top_level_key_sequence_is_rejected() -> None:
    ...
    with pytest.raises(AcceptanceSchemaError, match="declared order"):
        validate_stage2_acceptance(reordered)


def test_the_stage1_schema_literal_is_rejected() -> None:
    document = _synthetic_acceptance()
    document["schema_version"] = "radiosim.sci005.stage1-acceptance.v1"
    with pytest.raises(AcceptanceSchemaError, match="Stage-2 acceptance literal"):
        validate_stage2_acceptance(document)


def test_a_stage_one_declaration_is_rejected() -> None:
    document = _synthetic_acceptance()
    document["stage"] = 1
    with pytest.raises(AcceptanceSchemaError, match="integer 2"):
        validate_stage2_acceptance(document)


def test_a_stage1_evidence_path_is_rejected() -> None:
    document = _synthetic_acceptance()
    document["evidence_artifact_path"] = "docs/development/sci005_stage1_evidence.json"
    with pytest.raises(AcceptanceSchemaError, match="frozen Stage-2 path"):
        validate_stage2_acceptance(document)


def test_a_stage1_toolchain_path_is_rejected() -> None:
    document = _synthetic_acceptance()
    document["toolchain"]["acceptance_generator_path"] = (
        "tools/sci005_stage1_acceptance.py"
    )
    with pytest.raises(AcceptanceSchemaError, match="Stage-2 tool"):
        validate_stage2_acceptance(document)


def test_a_non_null_acceptance_commit_sha_is_rejected() -> None:
    """Section 8.2: ``acceptance_commit_sha`` is JSON null; ``U2`` binds ``A2``."""
    document = _synthetic_acceptance()
...
def test_the_stage1_self_reference_reason_is_rejected() -> None:
    document = _synthetic_acceptance()
...
def test_an_unsorted_unlock_array_is_rejected() -> None:
    """Section 8.2 freezes the sorted pair, so the M3 unlock leads."""
    document = _synthetic_acceptance()
    document["successor_unlocks"] = ["SCI005.U2", "SCI004.M3"]
    with pytest.raises(AcceptanceSchemaError, match="does not match the verdict"):
        validate_stage2_acceptance(document)
...
def test_a_stage1_oracle_identifier_is_refused() -> None:
    module = _tool()
    document = _synthetic_acceptance()
    document["rederived_oracles"][0]["oracle_id"] = "blocked_aperture_transform"
...
def test_the_tool_declares_the_frozen_stage2_constants() -> None:
    module = _tool()
    ...
    assert module.EVIDENCE_APPROVED_SOURCE_CONSTANT == "APPROVED_STAGE2_SOURCE_SHA"
    assert module.EVIDENCE_APPROVED_DIGEST_CONSTANT == (
        "APPROVED_STAGE2_EVIDENCE_ARTIFACT_SHA256"
    )


# --- Section 8.3: the amended ``U1 ->* D2`` succession -------------------------
...
def test_the_interval_table_is_the_header_enumerated_one() -> None:
    ...
    assert module.INTERVAL_COMMITS["3d60b6f428e9ac94407f50dfd9153aad27d5e098"][
        1
    ] == frozenset(
        {
            "docs/development/sci005_beam_physics_plan.md",
            "tests/unit/test_sci005_stage1_dependency.py",
        }
    )
    assert module.SUPERSEDED_RED_SLICE_PATHS == frozenset(
        {
...
def test_the_operative_design_edge_authenticates_against_real_history() -> None:
    """The whole starred edge, read from Git objects at the operative ``D2``."""
    module = _tool()
    design = _operative_design_commit(module)
    module.authenticate_design_edge(design)


def test_the_located_stage1_acceptance_and_status_commits_are_the_recorded_ones() -> (
    None
):
    module = _tool()
    design = _operative_design_commit(module)
    accepted = module.locate_stage1_acceptance_commit(design)
    assert accepted == "2281f2f00576abbc98a0a047ce192ad3013aa202"
    assert module.locate_status_successor(design, accepted) == (
        "d6eb4b083d4db704cd31abcf70e22cb745291e15"
    )
...
def test_an_interval_commit_touching_a_foreign_path_is_refused() -> None:
    ...
    with pytest.raises(module.AcceptanceError) as error:
        module.require_interval_kind(
            ...
            "status-prose",
            frozenset({"docs/changelog.rst", "src/radiosim/core/visibility.py"}),
        )
    assert error.value.prefix == "SCI005_ACCEPTANCE_DIFF_AUTHORITY"


def test_a_status_prose_commit_that_touched_source_is_refused() -> None:
    """The red slice's touch set is not admissible for a status-prose kind."""
    module = _tool()
    with pytest.raises(module.AcceptanceError) as error:
        module.require_interval_kind(
            "2a5d5aa7fe84a464368993a96c395fc444c2cedb",
            "status-prose",
            module.SUPERSEDED_RED_SLICE_PATHS,
        )
    assert error.value.prefix == "SCI005_ACCEPTANCE_DIFF_AUTHORITY"
...
def _operative_design_commit(module: Any) -> str:
    """Return the operative ``D2``: the newest header-recorded correction.

    The interval table names every commit between ``U1`` and ``D2``; ``D2`` is
    ...
    than to a second copy of the ``D2`` literal.
    """
    last_interval = "2a5d5aa7fe84a464368993a96c395fc444c2cedb"
    listing = module.run_git("rev-list", "--first-parent", "--parents", "HEAD")
    children = [
        line.split()[0]
        for line in listing.splitlines()
        if len(line.split()) == 2 and line.split()[1] == last_interval
    ]
...
def test_the_tool_reads_parents_not_peels() -> None:
    ...
    defect. Inside this tool the same confusion would turn ``R2^ == D2`` into a
...
def test_the_red_commit_must_be_a_direct_child_of_the_design_commit() -> None:
    """``R2^ == D2`` against real history, and a foreign parent is refused."""
    module = _tool()
    design = _operative_design_commit(module)
...
def _red_commit(module: Any, design: str) -> str:
    """Return ``R2``: the single first-parent child of the operative ``D2``."""
    listing = module.run_git("rev-list", "--first-parent", "--parents", "HEAD")
    children = [
        line.split()[0]
        for line in listing.splitlines()
    ...
    if not children:
        pytest.skip("R2 has not been cut yet; the D2 -> R2 edge cannot exist")
    assert len(children) == 1, children
    return children[0]
