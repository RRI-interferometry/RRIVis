...
**The ``D1`` acceptance record.** The operative ``D1`` is the *last* of a
three-deep chain of independently accepted design commits. The lineage matters
because Section 7.1 binds exactly one of them, and because each link was
opened by a defect the previous design could not answer -- so the chain is the
record of what was found, not bookkeeping.
...
that owner landed at ``1222f69a5ea0a6e008e38b7bfd4ba2d7f6c168d5``, after its
exact pre-landing bytes and diff took separate fresh independent governance and
computational ``ACCEPT`` verdicts on 2026-08-14, each reconfirmed after a
single-sentence Section 3.5 precision fix.

Third, Stage-1 red-test authoring stopped again, this time on a proof that
Section 3.4.1's frozen resource caps and its own frozen Hermite floor could not
both be satisfied: on this module's own oracle fixture the shifted-wavevector
rule asked for 4.92 times the frozen ``2**20`` wavevector bound, the base
coherent transform alone needed 158,400 aperture nodes against a 65,536 bound,
and the fixture's largest retained Poisson order could not have converged at
*any* allowed Hermite order. The cause was the quadrature variable, not the
constants: the shifted-wavevector rule costs ``O(S*J*(D/L)**4)`` and is
tractable only for ``L`` of order ``D``, which is exactly the regime where
scattered power degenerates to its own closed ``L -> infinity`` identity, while
real reflector surfaces are panel-correlated with ``L << D``. The bounded
quadrature-domain correction that evaluates the identical frozen mixture in the
separation variable landed at ``c6a5ce90ae3160150b1699f97b45bb693d4ed886``,
after its exact pre-landing bytes and diff took separate fresh independent
physics and computational ``ACCEPT`` verdicts on 2026-08-15, each reconfirmed
after a three-edit delta.

Every one of those digests -- each commit's landed memo blob and landed
parent-relative diff, and each correction's *pre-landing* file and diff pins --
is authenticated below against Git objects and against the memo's own header
text; see :data:`SCI005_DESIGN_SUCCESSION`. Both corrections landed with only a
record sentence added, mirroring the
``8935052cc4e49e3ff7bb92f645d03cee6b9e8ad2`` precedent, which is why each
landed blob differs from the pre-landing bytes its reviews bound.

The last commit in that chain supersedes its predecessors as "the independently
...
APPROVED_SCI005_D1_SHA = "c6a5ce90ae3160150b1699f97b45bb693d4ed886"
...
D1_MEMO_BLOB_SHA256 = "ab308351d78f48c169aeca3458fe783a48f1a215653f275eb57b9d9dbc92129f"
#: The landed ``D1`` parent-relative diff over the memo path.
D1_REVIEWED_DIFF_SHA256 = (
    "5c0ff76747ae40f52ba9283a2610f36b478552f16517dbf0dfd93670c474b388"
)


class _DesignCommit(NamedTuple):
    """One independently accepted design commit in the ``D1`` succession."""

    sha: str
    landed_blob_sha256: str
    landed_diff_sha256: str
    #: The exact bytes the two fresh reviews bound, when the commit landed with
    #: a record sentence added afterwards.  ``None`` when the reviews were run
    #: against the landed commit itself.
    pre_landing_blob_sha256: str | None
    pre_landing_diff_sha256: str | None
    label: str


#: The complete three-deep ``D1`` succession, oldest first.  The last entry is
#: the operative gate and must equal :data:`APPROVED_SCI005_D1_SHA`.
SCI005_DESIGN_SUCCESSION: tuple[_DesignCommit, ...] = (
    _DesignCommit(
        sha="58e7fb3d09dbcaec6f8201a778a653b55996c1aa",
        landed_blob_sha256=(
        ...
        pre_landing_blob_sha256=None,
        pre_landing_diff_sha256=None,
        label="acceptance-succession amendment",
    ),
    _DesignCommit(
        sha="1222f69a5ea0a6e008e38b7bfd4ba2d7f6c168d5",
        landed_blob_sha256=(
            "20b4cda18f4cadc1f6ba3d9e096a8831e3351c4e78816be2af6297bff8901b5a"
        ),
        landed_diff_sha256=(
            "32199b2e505eac0dba90c1ae412992914e70bfdab8d48de02838bc4b4e0d87ee"
        ),
        pre_landing_blob_sha256=(
            "2fb7c2f328f343fb6054a183e35aa58a7795f6a5ab6efe3c2ea29660a3aae001"
        ),
        pre_landing_diff_sha256=(
            "3d772d7be5d290bd68e64211c9cb5ffd065ec56e1ca7a430810de26172b3e347"
        ),
        label="support-leg ownership correction",
    ),
    _DesignCommit(
        sha=APPROVED_SCI005_D1_SHA,
        landed_blob_sha256=D1_MEMO_BLOB_SHA256,
        landed_diff_sha256=D1_REVIEWED_DIFF_SHA256,
        pre_landing_blob_sha256=(
            "09b185c4d741e31d65261c8953755adfc83f34e9ff7ea5cf16a75f00b8e75489"
        ),
        pre_landing_diff_sha256=(
            "f5fab63418b4ab13a1d8303a4d2c356640739f88131fd0780d598d2256dc8fa4"
        ),
        label="Ruze quadrature-domain correction",
    ),
)
...
def test_the_design_succession_is_a_strictly_ordered_authenticated_chain() -> None:
    """Each accepted design commit supersedes its predecessor; none is erased.

    Every predecessor's own two ``ACCEPT`` verdicts are what let the next link
    be a *bounded* correction rather than a fresh design, so every link's exact
    bytes stay reachable and authenticated even though Section 7.1 binds only
    the last one.
    """
    assert len(SCI005_DESIGN_SUCCESSION) == 3
    assert SCI005_DESIGN_SUCCESSION[-1].sha == APPROVED_SCI005_D1_SHA
    assert len({entry.sha for entry in SCI005_DESIGN_SUCCESSION}) == 3

    for older, newer in zip(
        SCI005_DESIGN_SUCCESSION[:-1], SCI005_DESIGN_SUCCESSION[1:], strict=True
    ):
        assert _is_ancestor(older.sha, newer.sha)
        assert not _is_ancestor(newer.sha, older.sha)

    for entry in SCI005_DESIGN_SUCCESSION:
        blob = _git_blob(entry.sha, DESIGN_MEMO_PATH)
        assert hashlib.sha256(blob).hexdigest() == entry.landed_blob_sha256, entry.label
        assert _memo_diff_digest(entry.sha) == entry.landed_diff_sha256, entry.label
        # Section 7.1 design-only authority: each link touches the memo alone.
        changed = _git(
            "diff-tree", "--no-commit-id", "--name-only", "-r", entry.sha
        ).split()
        assert changed == [DESIGN_MEMO_PATH], entry.label


def test_every_recorded_review_digest_appears_in_the_accepted_memo_header() -> None:
    """The memo's own header is the primary record of what the reviews bound.

    A correction that landed with a record sentence added cannot have its
    reviewed *pre-landing* bytes recovered from Git -- those bytes were never
    committed. The header records them instead, so authenticating the pin means
    checking it against the accepted memo text rather than against an object.
    """
    memo = _git_blob(APPROVED_SCI005_D1_SHA, DESIGN_MEMO_PATH).decode("utf-8")

    recorded: list[str] = []
    for entry in SCI005_DESIGN_SUCCESSION:
        for digest in (entry.pre_landing_blob_sha256, entry.pre_landing_diff_sha256):
            if digest is not None:
                assert _is_lower_hex(digest, width=64), entry.label
                assert digest in memo, f"{entry.label}: {digest} is not recorded"
                recorded.append(digest)
    # Two corrections landed with a record sentence; the amendment did not.
    assert len(recorded) == 4
    assert len(set(recorded)) == 4
    # A pre-landing pin is never equal to the bytes that actually landed.
    for entry in SCI005_DESIGN_SUCCESSION:
        if entry.pre_landing_blob_sha256 is not None:
            assert entry.pre_landing_blob_sha256 != entry.landed_blob_sha256
            assert entry.pre_landing_diff_sha256 != entry.landed_diff_sha256
