...
class TestCombineModels:
    ...
    def test_combine_preserves_channel_widths_hz(self, precision):
        freqs = np.array([100e6, 101e6], dtype=np.float64)
        widths = np.array([1e6, 1e6], dtype=np.float64)
        npix = hp.nside2npix(8)

        def _model_with_widths(value: float) -> SkyModel:
            return SkyModel(
                healpix=HealpixData(
                    maps=np.full((len(freqs), npix), value, dtype=np.float32),
                    nside=8,
        ...
        assert combined.healpix.channel_widths_hz is not None
        np.testing.assert_array_equal(combined.healpix.channel_widths_hz, widths)

    def test_combine_rejects_mismatched_channel_widths_hz(self, precision):
        freqs = np.array([100e6, 101e6], dtype=np.float64)
        npix = hp.nside2npix(8)

        def _model(widths: np.ndarray, value: float) -> SkyModel:
            return SkyModel(
                healpix=HealpixData(
                    maps=np.full((len(freqs), npix), value, dtype=np.float32),
                    nside=8,
        ...
        sky_b = _model(np.array([5e5, 5e5]), value=50.0)
        with pytest.raises(ValueError, match="mismatched channel_widths_hz"):
            _combine_models(
                [sky_a, sky_b],
                representation=SkyFormat.HEALPIX,
                precision=precision,
    ...
    def test_combine_rejects_asymmetric_channel_widths_hz(self, precision):
        freqs = np.array([100e6, 101e6], dtype=np.float64)
        npix = hp.nside2npix(8)
        sky_with = SkyModel(
            healpix=HealpixData(
                maps=np.full((len(freqs), npix), 100.0, dtype=np.float32),
                nside=8,
                frequencies=freqs,
                channel_widths_hz=np.array([1e6, 1e6]),
                coordinate_frame="icrs",
            ),
            reference_frequency=float(freqs[0]),
        ...
        with pytest.raises(ValueError, match="asymmetric channel_widths_hz"):
            _combine_models(
                [sky_with, sky_without],
                representation=SkyFormat.HEALPIX,
                precision=precision,
            )

    def test_combine_point_only_to_healpix_has_no_channel_widths(self, precision):
        sky_a = make_point_model(5, precision=precision, seed=1)
        sky_b = make_point_model(5, precision=precision, seed=2)
        combined = _combine_models(
            [sky_a, sky_b],
            representation=SkyFormat.HEALPIX,
            nside=8,
