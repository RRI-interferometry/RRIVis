...
class TestStokesToCoherency:
    ...
    def test_unpolarized_source(self):
        ...
        stokes_I = 10.0
        C = stokes_to_coherency(stokes_I)

        # Should be diagonal with I/2 on each element
        assert np.allclose(C[0, 0].real, stokes_I / 2), "C[0,0] should be I/2"
        assert np.allclose(C[1, 1].real, stokes_I / 2), "C[1,1] should be I/2"
        ...
        assert np.allclose(np.trace(C).real, stokes_I), "Trace(C) must equal I"

    def test_fully_Q_polarized(self):
        """Fully Q-polarized: Q=I, U=V=0 (all in X feed)."""
        stokes_I, stokes_Q = 10.0, 10.0
        C = stokes_to_coherency(stokes_I, stokes_Q=stokes_Q)

        # All power in XX
        assert np.allclose(C[0, 0].real, stokes_I), "C[0,0] = I (all in X)"
        assert np.allclose(C[1, 1].real, 0), "C[1,1] = 0 (none in Y)"
        assert np.allclose(C[0, 1], 0), "Off-diagonal zero"
        assert np.allclose(np.trace(C).real, stokes_I), "Trace = I"

    def test_africanus_V_convention(self):
        """Verify Africanus convention: C[0,1] = (U - iV)/2."""
        stokes_I, stokes_U, stokes_V = 1.0, 0.0, 1.0  # Pure circular polarization

        C = stokes_to_coherency(stokes_I, stokes_U=stokes_U, stokes_V=stokes_V)

        # Africanus: C[0,1] = (U - iV)/2 = (0 - i)/2 = -i/2
        expected_01 = (stokes_U - 1j * stokes_V) / 2
    ...
    def test_energy_conservation(self):
        ...
        for stokes_I, stokes_Q, stokes_U, stokes_V in test_cases:
            C = stokes_to_coherency(stokes_I, stokes_Q, stokes_U, stokes_V)
            trace = np.trace(C).real
            assert np.allclose(trace, stokes_I, rtol=1e-10), (
                f"Energy not conserved for I={stokes_I}, Q={stokes_Q}, U={stokes_U}, V={stokes_V}"
            )

    def test_hermitian_property(self):
        """Coherency matrix must be Hermitian: C = C^H."""
        stokes_I, stokes_Q, stokes_U, stokes_V = 5.0, 1.0, -0.5, 0.3
        C = stokes_to_coherency(stokes_I, stokes_Q, stokes_U, stokes_V)
    ...
    def test_array_broadcasting(self):
        ...
        Nsrc = 100
        stokes_I = np.random.uniform(1, 10, Nsrc)
        stokes_Q = np.random.uniform(-stokes_I, stokes_I)
        stokes_U = np.zeros(Nsrc)
        stokes_V = np.zeros(Nsrc)
        ...
        assert np.allclose(traces, stokes_I), "Energy not conserved in array"
...
class TestCoherencyToStokes:
    ...
    def test_roundtrip_unpolarized(self):
        ...
        assert np.allclose(
            [stokes_I, stokes_Q, stokes_U, stokes_V], [I2, Q2, U2, V2], rtol=1e-10
        )

    def test_roundtrip_polarized(self):
        """Round-trip with full polarization."""
        stokes_I, stokes_Q, stokes_U, stokes_V = 10.0, 2.0, -3.0, 1.5
    ...
    def test_roundtrip_array(self):
        ...
        stokes_I = np.random.uniform(1, 10, Nsrc)
        stokes_Q = np.random.uniform(-stokes_I, stokes_I)
        stokes_U = np.random.uniform(
            -np.sqrt(stokes_I**2 - stokes_Q**2), np.sqrt(stokes_I**2 - stokes_Q**2)
        )
        stokes_V = np.random.uniform(-1, 1, Nsrc)

        C = stokes_to_coherency(stokes_I, stokes_Q, stokes_U, stokes_V)
        I2, Q2, U2, V2 = coherency_to_stokes(C)

        assert np.allclose(stokes_I, I2, rtol=1e-10)
        assert np.allclose(stokes_Q, Q2, rtol=1e-10)
        assert np.allclose(stokes_U, U2, rtol=1e-10)
        assert np.allclose(stokes_V, V2, rtol=1e-10)
...
class TestApplyJonesMatrices:
    ...
    def test_hermitian_visibility(self):
        ...
        vis_H = np.conj(vis.T)
        assert np.allclose(vis, vis_H, atol=1e-10), (
            "Autocorrelation visibility must be Hermitian"
        )
    ...
    def test_array_broadcasting(self):
        ...
        Nsrc = 10
        stokes_I = np.random.uniform(1, 10, Nsrc)
        C = stokes_to_coherency(stokes_I)  # Shape: (10, 2, 2)
...
class TestVisibilityToCorrelations:
    ...
    def test_unpolarized_ideal_instrument(self):
        ...
        assert np.allclose(corr["XX"], stokes_I / 2), "XX should be I/2"
        assert np.allclose(corr["YY"], stokes_I / 2), "YY should be I/2"
        ...
        assert np.allclose(corr["I"], stokes_I), "I = XX + YY (energy conservation)"
    ...
    def test_fully_Q_polarized(self):
        ...
        # All power in XX
        assert np.allclose(corr["XX"], stokes_I), "XX = I for Q=I"
        assert np.allclose(corr["YY"], 0), "YY = 0 for Q=I"
        assert np.allclose(corr["I"], stokes_I), "Total intensity still I"
    ...
    def test_stokes_I_no_division_by_2(self):
        ...
        for stokes_I, stokes_Q, stokes_U, stokes_V in test_cases:
            ...
            assert np.allclose(corr["I"], stokes_I, rtol=1e-10), (
                f"Failed for I={stokes_I}, Q={stokes_Q}, U={stokes_U}, V={stokes_V}"
            )
...
class TestStokesIOnlyVisibility:
    ...
    def test_matches_full_calculation(self):
        ...
        V_fast = stokes_I_only_visibility(J_i, J_j, stokes_I)
    ...
    def test_leakage_with_unpolarized(self):
        ...
        V = stokes_I_only_visibility(J_leaky, J_leaky, stokes_I)
...
class TestEdgeCases:
    ...
    def test_zero_intensity(self):
        ...
        C = stokes_to_coherency(stokes_I=0.0)
