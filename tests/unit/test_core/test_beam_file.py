...
class TestAzimuthConversion:
    ...
    def test_cardinal_directions(self):
        ...
        assert np.allclose(astropy_az_to_uvbeam_az(0.0), np.pi / 2)

        # East in Astropy (90°) → East in UVBeam (0°)
        assert np.allclose(astropy_az_to_uvbeam_az(np.pi / 2), 0.0)

        # South in Astropy (180°) → South in UVBeam (270°)
        assert np.allclose(astropy_az_to_uvbeam_az(np.pi), 3 * np.pi / 2)

        # West in Astropy (270°) → West in UVBeam (180°)
        assert np.allclose(astropy_az_to_uvbeam_az(3 * np.pi / 2), np.pi)

    def test_intermediate_angles(self):
        """Test conversion of non-cardinal angles."""
        # Northeast in Astropy (45°) → 45° in UVBeam
        assert np.allclose(astropy_az_to_uvbeam_az(np.pi / 4), np.pi / 4)

        # Southeast in Astropy (135°) → 315° in UVBeam
        assert np.allclose(astropy_az_to_uvbeam_az(3 * np.pi / 4), 7 * np.pi / 4)
...
class TestBeamFITSHandler:
    ...
    def test_load_beam_with_correct_units(
        ...
        BeamFITSHandler("/mock/path/beam.fits", mock_config, mock_logger)
    ...
    def test_jones_matrix_ordering(
        ...
        jones = handler.get_jones_matrix(
            alt_rad=alt_rad, az_rad=az_rad, freq_hz=freq_hz, location=None, time=None
        )
...
class TestBeamManager:
    ...
    def mock_config_per_antenna_config(self):
        ...
        return {
            "beams": {
                "use_beam_file": True,
                "use_different_beams": True,
                "beam_files": {0: "/mock/beam1.fits", 1: "/mock/beam2.fits"},
                "beams_per_antenna": {0: 0, 1: 1},
                "beam_za_max_deg": 90.0,
                "beam_freq_buffer_mhz": 1.0,
            },
            "obs_frequency": {"freq_min_MHz": 49.0, "freq_max_MHz": 51.0},
    ...
    def test_analytic_mode(
        self, mock_logger, mock_config_analytic, mock_antenna_data_no_beam_ids
    ):
        """Test analytic mode - should not load any beams."""
        manager = BeamManager(
            mock_config_analytic, mock_antenna_data_no_beam_ids, mock_logger
        )
    ...
    def test_shared_mode(
        ...
        manager = BeamManager(
            mock_config_shared, mock_antenna_data_no_beam_ids, mock_logger
        )
    ...
    def test_per_antenna_from_layout_with_beam_ids(
        ...
        manager = BeamManager(
            mock_config_per_antenna_layout, mock_antenna_data_with_beam_ids, mock_logger
        )
    ...
    def test_per_antenna_from_layout_without_beam_ids_raises_error(
        self, mock_logger, mock_config_per_antenna_layout, mock_antenna_data_no_beam_ids
    ):
        """
        ...
        """
        with pytest.raises(
            ValueError, match="use_different_beams=True but no beam assignments found"
        ):
            BeamManager(
                mock_config_per_antenna_layout,
                mock_antenna_data_no_beam_ids,
    ...
    def test_per_antenna_from_config(
        ...
        manager = BeamManager(
            mock_config_per_antenna_config, mock_antenna_data_no_beam_ids, mock_logger
        )
    ...
    def test_realistic_rime_workflow(
        ...
        def create_mock_handler(beam_path, cfg, logger, **kwargs):
            ...
            if "beam_A" in beam_path:
                ...
                jones = np.array([[1.0 + 0j, 0.1 + 0j], [0.1 + 0j, 0.5 + 0j]])
            else:  # beam_B
                # Beam B: stronger in Y-pol
                jones = np.array([[0.5 + 0j, 0.1 + 0j], [0.1 + 0j, 1.0 + 0j]])
        ...
        assert not np.allclose(E1, E2), (
            "Different beams should give different Jones matrices"
        )
        ...
        from rrivis.core.polarization import apply_jones_matrices, stokes_to_coherency

        C = stokes_to_coherency(stokes_I=1.0, stokes_Q=0, stokes_U=0, stokes_V=0)
