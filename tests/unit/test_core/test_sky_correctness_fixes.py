...
# -----------------------------------------------------------------------------
# Fix 18 — channel_widths_hz on HealpixData validates and round-trips.
# -----------------------------------------------------------------------------


def test_healpix_data_accepts_channel_widths() -> None:
    nside = 4
    npix = hp.nside2npix(nside)
    hd = HealpixData(
        maps=np.zeros((2, npix), dtype=np.float32),
        nside=nside,
        frequencies=np.array([100e6, 150e6]),
        channel_widths_hz=np.array([1e6, 1e6]),
    )
    assert hd.channel_widths_hz is not None
    assert hd.channel_widths_hz.shape == (2,)


def test_healpix_data_channel_widths_wrong_shape_raises() -> None:
    nside = 4
    npix = hp.nside2npix(nside)
    with pytest.raises(ValueError, match="channel_widths_hz"):
        HealpixData(
            maps=np.zeros((2, npix), dtype=np.float32),
            nside=nside,
            frequencies=np.array([100e6, 150e6]),
            channel_widths_hz=np.array([1e6, 1e6, 1e6]),
        )


def test_healpix_data_channel_widths_must_be_positive() -> None:
    nside = 4
    npix = hp.nside2npix(nside)
    with pytest.raises(ValueError, match="channel_widths_hz"):
        HealpixData(
            ...
            nside=nside,
            frequencies=np.array([100e6, 150e6]),
            channel_widths_hz=np.array([1e6, -1.0]),
        )
