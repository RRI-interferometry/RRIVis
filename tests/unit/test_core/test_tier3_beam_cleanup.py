...
def test_removed_names_are_absent_from_public_module_surfaces() -> None:
    module_names = (
        ...
        "radiosim.core.jones.beam.analytic.feed",
    )
...
def test_analytic_public_api_is_exact_and_preserves_object_identity() -> None:
    ...
    defining_modules = (aperture, taper, feed, numerical_hpbw)
...
def test_retained_feed_and_reflector_primitives_are_numerically_sane() -> None:
    from radiosim.core.jones.beam.analytic import (
        cassegrain_angle,
        compute_edge_angle,
        corrugated_horn_pattern,
    ...
    assert corrugated_horn_pattern(angles)[0] == pytest.approx(1.0)
    e_plane, h_plane = open_waveguide_pattern(angles)
    assert e_plane[0] == pytest.approx(1.0)
    assert h_plane[0] == pytest.approx(1.0)
    assert dipole_ground_plane_pattern(angles)[0] == pytest.approx(1.0)
