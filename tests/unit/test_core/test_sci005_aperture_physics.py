...
def test_surface_height_phase_uses_the_negative_four_pi_over_lambda_convention() -> (
    ...
    # A quarter-wave of surface height is a half-wave of path: exp(-i*pi/2).
    _assert_within_frozen_tolerance(observed[1], -1j)
...
def test_authored_height_is_half_reflected_opd_at_any_incidence(
    ...
    _assert_within_frozen_tolerance(
        phase, np.exp(-1j * 2.0 * np.pi * excess_path_m / wavelength_m)
    )
...
def test_the_quadrature_seed_grows_with_far_field_and_surface_phase_bandwidth() -> None:
    """Section 3.3: both ``Q_max`` and ``kappa*H`` enter the seed."""
    ...
    assert low == 16
    assert far_field > low
    assert surface == far_field
