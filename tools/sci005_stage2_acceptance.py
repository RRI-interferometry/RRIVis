#!/usr/bin/env python
"""Generate and verify the SCI-005 Stage-2 independent acceptance record.
...
``docs/development/sci005_stage2_acceptance.schema.json`` stays the normative
...
Generation is all-or-rollback and owns the complete admissible pre-``A2`` diff::

    pixi run python tools/sci005_stage2_acceptance.py generate \\
...
clean exact ``E2``, first invokes the active evidence validator, and for an
``ACCEPT`` prepares both the previously absent canonical JSON and the phase
validator with exactly ``APPROVED_EVIDENCE_SHA: None -> E2`` and
...
dependency::
    ...
    pixi run python tools/sci005_stage2_acceptance.py verify \\
      --acceptance-commit <A2> --descendant <SHA-or-HEAD>

Beyond the Stage-1 checks it authenticates the amended Section 8.3 succession
from Git objects alone: ``R2^ == D2``; ``A1`` located as the unique commit
introducing the Stage-1 acceptance artifact and authenticated by the Stage-1
approved constants; ``U1`` located as the unique commit on ``D2``'s first-parent
ancestry whose direct parent is ``A1``, satisfying the committed Stage-1
``verify-status`` form; ``A1`` an ancestor of ``D2`` through the committed
Stage-1 ``verify`` form; and every commit in ``U1..D2`` other than ``D2``
matched against the header-enumerated three-kind interval rule with its own
recorded touch set. The operative ``D2`` itself touches only the design memo.
...
A status successor is checked before and after commit with::
    ...
    pixi run python tools/sci005_stage2_acceptance.py verify-status \\
      --acceptance-commit <A2> --status-commit <U2-or-INDEX>
"""
...
ACCEPTANCE_ARTIFACT = "docs/development/sci005_stage2_acceptance.json"
ACCEPTANCE_SCHEMA = "docs/development/sci005_stage2_acceptance.schema.json"
ACCEPTANCE_VALIDATOR = "tests/unit/test_sci005_stage2_acceptance.py"
EVIDENCE_ARTIFACT = "docs/development/sci005_stage2_evidence.json"
EVIDENCE_SCHEMA = "docs/development/sci005_stage2_evidence.schema.json"
EVIDENCE_GENERATOR = "tools/sci005_stage_evidence.py"
EVIDENCE_VALIDATOR = "tests/unit/test_sci005_evidence.py"
ACCEPTANCE_GENERATOR = "tools/sci005_stage2_acceptance.py"

#: Section 7.5's two Stage-2 approved evidence constants, named here because
#: ``verify`` authenticates the retained evidence artifact against the pins the
#: evidence validator itself carries at ``E2``.
EVIDENCE_APPROVED_SOURCE_CONSTANT = "APPROVED_STAGE2_SOURCE_SHA"
EVIDENCE_APPROVED_DIGEST_CONSTANT = "APPROVED_STAGE2_EVIDENCE_ARTIFACT_SHA256"

#: The Stage-1 retained surface the amended Section 8.3 edge is anchored on.
STAGE1_ACCEPTANCE_ARTIFACT = "docs/development/sci005_stage1_acceptance.json"
STAGE1_ACCEPTANCE_VALIDATOR = "tests/unit/test_sci005_stage1_acceptance.py"
STAGE1_ACCEPTANCE_TOOL = "tools/sci005_stage1_acceptance.py"
STAGE1_APPROVED_EVIDENCE_CONSTANT = "APPROVED_EVIDENCE_SHA"
STAGE1_APPROVED_ARTIFACT_CONSTANT = "APPROVED_ACCEPTANCE_ARTIFACT_SHA256"

DESIGN_MEMO = "docs/development/sci005_beam_physics_plan.md"
DEPENDENCY_VALIDATOR = "tests/unit/test_sci005_stage1_dependency.py"

SELF_REFERENCE_REASON = "self-reference: U2 and SCI004.M3 bind the containing A2 commit"
REJECT_REASON = "not-applicable: REJECT creates no A commit"
UNLOCKS = ["SCI004.M3", "SCI005.U2"]
...
REQUIRED_ORACLES = frozenset(
    {
        "mechanical_feed_rotation",
        "native_feed_factorization",
        "noncommuting_chain_order",
        "squint_frequency_law",
    }
)
...
#: The four Section 7.3 test paths the header-recorded superseded red slice
#: cut, and the only paths that kind of commit may touch.
SUPERSEDED_RED_SLICE_PATHS = frozenset(
    {
...
#: Section 8.3's observed ``U1..D2`` interval, transcribed from this memo's own
#: header records: each commit's SHA, its kind, and the exact paths its header
#: record names. A commit in the interval that this table does not name
#: invalidates the starred edge.
INTERVAL_COMMITS: dict[str, tuple[str, frozenset[str]]] = {
    # The changelog heading repair U1's own Sphinx-strict failure forced.
    "3e336095009e72bf4ae6064d5e97d381e063258f": (
        STATUS_PROSE_KIND,
        frozenset({"docs/changelog.rst"}),
    ),
    # The superseded original Stage-2 gate: the memo amendment plus the one
    # authorized four-line dependency-validator deletion.
    "3d60b6f428e9ac94407f50dfd9153aad27d5e098": (
        SUPERSEDED_DESIGN_KIND,
        frozenset({DESIGN_MEMO, DEPENDENCY_VALIDATOR}),
    ),
    # The superseded heading-and-binding correction: memo only.
    "0c378153f5d5fa46574ca334d109b40e102f12e6": (
        SUPERSEDED_DESIGN_KIND,
        frozenset({DESIGN_MEMO}),
    ),
    # The superseded first red slice, reopened for a governed re-cut.
    "2a5d5aa7fe84a464368993a96c395fc444c2cedb": (
        SUPERSEDED_RED_SLICE_KIND,
        SUPERSEDED_RED_SLICE_PATHS,
    ),
}
...
def validate_acceptance_document(document: Any) -> None:
    ...
    if document["schema_version"] != "radiosim.sci005.stage2-acceptance.v1":
        raise AcceptanceError(SCHEMA, "schema_version is not the Stage-2 literal")
    if document["stage"] != 2 or isinstance(document["stage"], bool):
        raise AcceptanceError(SCHEMA, "stage must be the integer 2")
    ...
    if document["acceptance_commit_sha"] is not None:
        raise AcceptanceError(
            SCHEMA, "acceptance_commit_sha is JSON null; U2 binds the containing A2"
        )
...
# --- the amended Section 8.3 ``U1 ->* D2`` succession --------------------------
...
def locate_stage1_acceptance_commit(descendant: str) -> str:
    """Return ``A1``: the unique commit introducing the Stage-1 artifact.

    Section 8.3 requires ``A1`` to be *located*, not assumed, and then
    authenticated by the Stage-1 approved constants rather than by name.
    """
    introductions = run_git(
        ...
        descendant,
        "--",
        STAGE1_ACCEPTANCE_ARTIFACT,
    ).split()
    if len(introductions) != 1:
        raise AcceptanceError(
            ANCESTRY,
            f"{STAGE1_ACCEPTANCE_ARTIFACT} must be introduced exactly once on the "
            f"ancestry of {descendant}; observed {introductions}",
        )
    accepted = introductions[0]

    validator = git_show(accepted, STAGE1_ACCEPTANCE_VALIDATOR).decode("utf-8")
    approved_evidence = read_approved_constant(
        validator, STAGE1_APPROVED_EVIDENCE_CONSTANT, STAGE1_ACCEPTANCE_VALIDATOR
    )
    approved_artifact = read_approved_constant(
        validator, STAGE1_APPROVED_ARTIFACT_CONSTANT, STAGE1_ACCEPTANCE_VALIDATOR
    )
    payload = git_show(accepted, STAGE1_ACCEPTANCE_ARTIFACT)
    if sha256_bytes(payload) != approved_artifact:
        raise AcceptanceError(
            DIGEST,
            "the located A1 artifact does not match the Stage-1 approved digest",
        )
    parent = parent_of(accepted)
    if parent != approved_evidence:
        raise AcceptanceError(
            ANCESTRY,
            f"A1^ is {parent}, not the Stage-1 approved evidence commit "
            f"{approved_evidence}",
        )
    document = parse_strict_json(
        payload.decode("utf-8"), STAGE1_ACCEPTANCE_ARTIFACT, SCHEMA
    )
    if document.get("evidence_commit_sha") != approved_evidence:
        raise AcceptanceError(
            DIGEST, "the located A1 artifact names a foreign evidence commit"
        )
    if document.get("verdict") != "ACCEPT":
        raise AcceptanceError(VERDICT, "the located A1 artifact is not an ACCEPT")
    return accepted


def run_stage1_tool(*arguments: str) -> None:
    """Run the committed Stage-1 acceptance tool and require a zero exit."""
    completed = subprocess.run(
        [sys.executable, str(REPOSITORY_ROOT / STAGE1_ACCEPTANCE_TOOL), *arguments],
        cwd=str(REPOSITORY_ROOT),
        capture_output=True,
        text=True,
        check=False,
    ...
    if completed.returncode != 0:
        raise AcceptanceError(
            ANCESTRY,
            f"the committed Stage-1 {arguments[0]} form failed: "
            f"{completed.stderr.strip()}",
        )


def locate_status_successor(design_commit: str, stage1_acceptance: str) -> str:
    """Return ``U1``: the unique first-parent child of ``A1`` under ``D2``."""
    listing = run_git("rev-list", "--first-parent", "--parents", design_commit)
    candidates: list[str] = []
    for line in listing.splitlines():
        fields = line.split()
        if len(fields) == 2 and fields[1] == stage1_acceptance:
            candidates.append(fields[0])
    if len(candidates) != 1:
        raise AcceptanceError(
            ANCESTRY,
            "exactly one commit on D2's first-parent ancestry must directly "
            f"parent A1; observed {candidates}",
        )
    return candidates[0]
...
def require_interval_kind(commit: str, kind: str, recorded: frozenset[str]) -> None:
    ...
    if kind == STATUS_PROSE_KIND:
        ...
        outside = sorted(touched - SUPERSEDED_RED_SLICE_PATHS)
        if outside:
            raise AcceptanceError(
                DIFF_AUTHORITY,
                f"superseded red-slice commit {commit} touches {outside}, which is "
                "not a Section 7.3 test path",
            )
...
def authenticate_design_edge(design_sha: str) -> None:
    """Authenticate the amended Section 8.3 starred edge ``U1 ->* D2``.

    Every fact here comes from Git objects. ``A1`` is located by artifact
    introduction and authenticated by the Stage-1 approved constants; ``U1`` is
    the unique first-parent child of ``A1`` under ``D2`` and must satisfy the
    committed Stage-1 ``verify-status`` form; ``A1`` must be an ancestor of
    ``D2`` through the committed Stage-1 ``verify`` form; and every commit in
    ``U1..D2`` other than ``D2`` must be named by the header-enumerated interval
    table and touch exactly the paths its kind allows. Because no admissible
    kind may touch the retained Stage-1 evidence and acceptance surface, that
    surface is byte-identical to ``U1`` by construction.
    """
    stage1_acceptance = locate_stage1_acceptance_commit(design_sha)
    if not is_ancestor(stage1_acceptance, design_sha):
        raise AcceptanceError(
            ANCESTRY, f"A1 {stage1_acceptance} is not an ancestor of D2 {design_sha}"
        )
    run_stage1_tool(
        "verify", "--acceptance-commit", stage1_acceptance, "--descendant", design_sha
    )
    status_successor = locate_status_successor(design_sha, stage1_acceptance)
    run_stage1_tool(
        "verify-status",
        "--acceptance-commit",
        stage1_acceptance,
    ...
    if sorted(interval) != sorted(INTERVAL_COMMITS):
        raise AcceptanceError(
            ANCESTRY,
            "the observed U1..D2 interval is not the header-enumerated one; "
    ...
    if design_touched != frozenset({DESIGN_MEMO}):
        raise AcceptanceError(
            DIFF_AUTHORITY,
            f"the operative D2 must touch only {DESIGN_MEMO}; observed "
            f"{sorted(design_touched)}",
        )


def authenticate_succession(design_sha: str, red_test_sha: str) -> None:
    """Authenticate ``R2^ == D2`` and then the starred ``U1 ->* D2`` edge.

    Section 8.3: ``D2`` is the unambiguous direct parent of ``R2``. The
    ...
    if design_sha == red_test_sha:
        raise AcceptanceError(
            ANCESTRY, "D2 and R2 are the same commit; Section 8.3 requires R2^ == D2"
        )
    observed = parent_of(red_test_sha)
    if observed != design_sha:
        raise AcceptanceError(
            ANCESTRY, f"R2^ is {observed}, not the recorded design commit {design_sha}"
        )
...
def verify(acceptance_commit: str, descendant: str) -> bytes:
    ...
    if parent != evidence_commit:
        raise AcceptanceError(
            ANCESTRY,
            f"A2^ is {parent}, not the bound evidence commit {evidence_commit}",
        )
    ...
    if approved_source != document["source_sha"]:
        raise AcceptanceError(
            DIGEST, "the Stage-2 approved source constant disagrees with the record"
        )
    if approved_digest != document["evidence_artifact_sha256"]:
        raise AcceptanceError(
            DIGEST, "the Stage-2 approved evidence digest disagrees with the record"
        )
    ...
    if changed != sorted([ACCEPTANCE_ARTIFACT, ACCEPTANCE_VALIDATOR]):
        raise AcceptanceError(
            DIFF_AUTHORITY,
            f"the E2..A2 diff must be exactly the two A2 paths; observed {changed}",
        )
...
def verify_status(acceptance_commit: str, status_commit: str) -> None:
    ...
    if status_commit == "INDEX":
        ...
        if parent != accepted:
            raise AcceptanceError(ANCESTRY, "U2^ must be the acceptance commit")
        changed = sorted(
...
def generate(review_record: Path, reject_output: Path | None) -> None:
    ...
    document: dict[str, Any] = {
        "schema_version": "radiosim.sci005.stage2-acceptance.v1",
        "stage": STAGE,
        "verdict": verdict,
    ...
    try:
        ...
        if changed != sorted([ACCEPTANCE_ARTIFACT, ACCEPTANCE_VALIDATOR]):
            raise AcceptanceError(
                DIFF_AUTHORITY,
                f"the working diff must be exactly the two A2 paths; observed {changed}",
            )
    except Exception:
