...
**One row added at Tier 6I independent acceptance (2026-07-31), no decision
change.** Found while reproducing the committed record set; does not
contradict any binding 6I text and does not block acceptance.

| `time_backend_call`'s `tracemalloc`-based `peak_host_bytes` under-represents the JAX backend's real working set, because XLA allocates its buffers natively rather than through Python's tracked allocator, so `tracemalloc` only sees the thin Python/NumPy wrapper layer around them | confirmed by reproducing the committed reference set: on the `point_scaled_4096_sources_4times` workload the NumPy record reports `peak_host_bytes = 6,115,963` while the JAX record for the *same* workload reports only `1,253,968` — smaller, not larger, despite JAX doing strictly more work (tracing plus the same arithmetic). `backend_memory_info` for JAX already carries a qualitative disclaimer (`"JAX manages memory automatically via XLA"`), but no record field, docstring, or documentation surface states that `peak_host_bytes` specifically is not a fair cross-backend memory comparison. Not a record-honesty defect — no field claims otherwise, and every mandatory §23 field is present and internally consistent — but a reader comparing `peak_host_bytes` across backends could conclude JAX uses less host memory, which the harness cannot actually support. Filed as a documentation-methodology note for whoever next revises the benchmark guide or the harness docstring: state plainly that `peak_host_bytes` is directly comparable only among backends that allocate through Python's own heap (NumPy, Dask), and that JAX's true working set is not observable this way. No slice is authorized to fix this now; it is recorded so it is a known limitation rather than a silent one |

**One row added at Tier 6J independent whole-tier review (2026-07-31,
REJECTED — blocking, not merely recorded).** Unlike every prior row in this
table, this one is the reason acceptance failed, not a discharged or
routed observation.

| R1/S1/S6-S10's bit-identity invariants are asserted and pinned per Python version only (`_ENVIRONMENT_KEY = f"py{sys.version_info[0]}{sys.version_info[1]}"`, `test_tier6_current_behavior.py`), with no axis for OS or CPU architecture, but the raw visibility-cube digest genuinely differs between `osx-arm64` (the only architecture any Tier 6 implementer or slice reviewer ever ran locally) and `x86_64` (`linux-64` and `osx-64`, both locked CI platforms) for identical Python versions, identical locked dependency versions, and identical source | confirmed directly from hosted CI logs, not inferred: at the exact Tier 6I acceptance SHA (`ff6920a`) and ten other consecutive Tier 6 commits from Tier 6A onward, `linux-64`/`osx-64` (both Python versions) fail `test_shipped_default_config_scientific_fingerprint`, `test_shipped_circular_receptor_config_scientific_fingerprint`, and all six `test_section_13_4_workload_fingerprints[...]` with `AssertionError` on the `_cube_digest` value itself, while `osx-arm64` (both versions), the quality job, and the backend-parity job pass every time; `linux-64/py3.11`'s own run confirms this is isolated to exactly those 8 tests, not an environment/install failure (`8 failed, 4211 passed, 150 warnings`). Pre-Tier-6 CI (`6928f59`) was fully green on all six OS/Python jobs, but Tier 5 asserted no cube-level bit-identity at all, so whether the divergence predates Tier 6 (always-present cross-architecture floating-point drift in vectorized trig/BLAS code, newly exercised by Tier 6A's pins) or was introduced within it (most plausibly 6D's accumulation restructure or 6H's batched-kernel compilation, both of which reorder floating-point operations) is undetermined and is the first step of the bounded repair task recorded in `Fix.md`'s dated 6J rejection entry. This single gap is why §37 criterion 25 fails and Tier 6 is not accepted as a whole; every other §37/§38 criterion this reviewer checked passed |
