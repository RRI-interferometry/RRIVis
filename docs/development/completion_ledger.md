...
fresh evidence/acceptance. Current time-manifest, nested certificate, phase/input
and source-cascade validation work must finish before the coupled evidence
...
The check rediscovers HEAD and accepts that changed commit identity. The variant
...
A bounded repair must retain the preflight source SHA through publication and
check it before and after raw tracked authentication. E lifecycle acceptance
remains pending that repair; no generated evidence has been accepted.
