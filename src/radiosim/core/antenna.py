...
coordinates for consistency throughout RadioSim.

Supported File Formats
----------------------
- **radiosim**: Native RadioSim text format with optional BeamID and diameter columns
...
Examples
--------
Basic usage with RadioSim format:
...
See Also
--------
radiosim.core.baseline : Baseline generation from antenna pairs
radiosim.core.beams : Beam pattern calculations using antenna properties
...
SUPPORTED_FORMATS = ["radiosim", "casa", "measurement_set", "uvfits", "mwa", "pyuvdata"]
...
def _convert_coordinates_to_enu(
    ...
    system used internally by RadioSim for all antenna position calculations.
...
def read_radiosim_format(file_path: str | Path) -> dict[int, dict[str, Any]]:
    """
    Read antenna positions from RadioSim native text format.

    The RadioSim format is a flexible whitespace-separated text format with
    ...
    *With BeamID (6+ columns):*
    ...
    Examples
    --------
    >>> antennas = read_radiosim_format("antenna_layout.txt")
    >>> print(antennas[0])
...
def read_antenna_positions(
    ...
    This is the main entry point for loading antenna layout data into RadioSim.
    ...
    file_path : str or Path
        Path to the antenna positions file.
    format_type : str, default="radiosim"
        File format identifier. Supported values:

        - ``"radiosim"``: Native RadioSim text format (default)
    ...
    See Also
    --------
    read_radiosim_format : Details on RadioSim format
    ...
    Examples
    --------
    Load antennas from RadioSim format (default):
