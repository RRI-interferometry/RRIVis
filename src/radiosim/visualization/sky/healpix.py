"""HEALPix sky plots — module-level functions."""
...
def plot_healpix_map(
    ...
    map_data, stokes_label = _get_stokes_map(sky, stokes, freq)
    ...
    auto = _auto_title(sky, f"{stokes_label} — {_freq_label(freq)}", frequency=None)
...
def plot_multifreq_grid(
    ...
    if frequencies is not None:
        ...
        all_freqs = sky.healpix.frequencies
    ...
    maps = []
    for freq in freqs:
        map_data, _ = _get_stokes_map(sky, stokes, freq)
        if map_data is None:
            maps.append(None)
        else:
    ...
    if title is not None:
        ...
        fig.suptitle(_auto_title(sky, f"Stokes {stokes}"), fontsize=14, y=0.98)
...
def plot_stokes(
    ...
    for i, stokes_name in enumerate(stokes_params):
        map_data, stokes_label = _get_stokes_map(sky, stokes_name, freq)
    ...
    suptitle = (
        title
        if title is not None
        else _auto_title(sky, f"Stokes IQUV — {_freq_label(freq)}")
    )
...
def plot_linear_polarization(
    ...
    *,
    ...
    a histogram of fractional polarisation ``P / |I|``.
    """
    import matplotlib.pyplot as plt
    ...
    _validate_plot_mode(sky, "healpix")
    freq = _resolve_plot_frequency(sky, frequency)
    pol = compute_linear_polarization(sky, frequency=freq)
    p_map = pol["P"]
    chi_map = pol["chi_deg"]
    ...
    suptitle = (
        title
        if title is not None
        else _auto_title(sky, f"Linear polarisation — {_freq_label(freq)}")
    )
    fig.suptitle(suptitle, fontsize=14, y=0.98)
