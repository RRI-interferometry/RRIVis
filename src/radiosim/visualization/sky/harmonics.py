"""Harmonic and cross-frequency HEALPix plots — module-level functions."""
...
def plot_angular_power_spectrum(
    ...
    if representation not in ("c_ell", "d_ell", "both"):
        raise ValueError(
            f"representation must be 'c_ell', 'd_ell' or 'both', got {representation!r}"
        )

    import matplotlib.pyplot as plt

    freq_arr = sky.healpix.frequencies if sky.healpix else None
    ...
    for freq in freqs_list:
        sky_map, _ = _get_stokes_map(sky, stokes, float(freq))
        if sky_map is None:
            continue
    ...
    fig.suptitle(
        title if title is not None else _auto_title(sky, "Angular power spectrum"),
        fontsize=14,
        y=1.02,
    )
...
def plot_cross_frequency_cell(
    ...
    ref_freq = (
        _resolve_plot_frequency(sky, ref_frequency)
        if ref_frequency is not None
        else float(freq_arr[0])
    )
    ref_idx = sky.healpix.resolve_frequency_index(ref_freq)
    ref_map, _ = _get_stokes_map(sky, stokes, ref_freq)
    if ref_map is None:
    ...
    for f_target in targets:
        m_target, _ = _get_stokes_map(sky, stokes, f_target)
        if m_target is None:
            continue
    ...
    ax.set_title(
        ...
        else _auto_title(sky, "Cross-frequency angular power spectrum")
    )
    plt.subplots_adjust(left=0.1, right=0.97, top=0.92, bottom=0.12)
    return fig


def plot_multipole_bands(
    sky: SkyModel,
    ...
    sky_map, _ = _get_stokes_map(sky, stokes, freq)
    if sky_map is None:
    ...
    fig.suptitle(
        title
        if title is not None
        else _auto_title(sky, f"Multipole-band decomposition — Stokes {stokes}", freq),
        fontsize=14,
        y=1.00,
    )
...
def plot_frequency_correlation(
    sky: SkyModel,
    pixel_stride: int = 1,
    ...
    freqs_mhz = np.asarray(sky.healpix.frequencies) / 1e6
    maps = _get_stokes_cube(sky, stokes)
    ...
    fig.suptitle(
        title
        if title is not None
        else _auto_title(sky, f"Frequency correlation — Stokes {stokes}"),
        fontsize=14,
        y=1.02,
    )
...
def plot_delay_spectrum(
    ...
    _validate_plot_mode(sky, "healpix")
    if sky.healpix is None or len(sky.healpix.frequencies) < 4:
        raise ValueError("delay_spectrum requires at least 4 observation frequencies.")

    import matplotlib.pyplot as plt

    freqs_hz = np.asarray(sky.healpix.frequencies)
    maps = _get_stokes_cube(sky, stokes)
    cube = maps[:, ::pixel_stride]
    ...
    fig.suptitle(
        title
        if title is not None
        else _auto_title(sky, f"Delay spectrum — Stokes {stokes}"),
        fontsize=14,
        y=1.02,
    )
    plt.subplots_adjust(left=0.08, right=0.98, top=0.88, bottom=0.14, wspace=0.25)
