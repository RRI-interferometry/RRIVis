"""Network connectivity detection for RadioSim.

Provides utilities to check internet connectivity and reachability of
specific services (VizieR, CASDA, pygdsm data, PySM3 data) that RadioSim
...
from radiosim.utils.logging import get_logger

logger = get_logger("radiosim.utils.network")
...
def get_network_status(offline: bool = False) -> NetworkStatus:
    ...
    This is the primary entry point used by :class:`~radiosim.api.simulator.Simulator`.
...
def get_required_services(sky_config: dict) -> dict[str, list[str]]:
    ...
    ----------
    sky_config : dict
        The ``sky_model`` section of the RadioSim configuration (as a dict).
