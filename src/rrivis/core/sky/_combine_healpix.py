# rrivis/core/sky/_combine_healpix.py
...
from ._data import PointSourceData, PointSpectrum
...
from .spectral import evaluate_point_flux_at_freq
...
def combine_healpix(
    ...
    for m in point_only_models:
        if m.has_point_sources and m.point is not None:
            ...
            ps_ref_freq = (
                m.point.ref_freq.astype(np.float64, copy=False)
                if m.point.ref_freq is not None and np.any(m.point.ref_freq > 0)
                else np.full(
                    m.point.n_sources,
                    m.reference_frequency or ref_frequency or 0.0,
                    dtype=np.float64,
                )
            )
