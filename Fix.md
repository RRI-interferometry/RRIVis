...
| SCI-006 | OPEN | RadioSim's local Stokes ``Q`` has the opposite sign to `pyuvsim`/`pyradiosky`'s for the same sky, BeamFITS feed convention (`x_orientation="east"`), and mount, discovered by the 7J `pyuvsim` cross-validation (`tests/crossvalidation/test_pyuvsim_comparison.py`, `output/crossvalidation/2026-08-02-pyuvsim-1.4.0.json`). WP-4's cited derivation and independent executable probe (`docs/development/sci006_polarization_convention.md`, 2026-08-08) now rule the current RadioSim binding non-normative: IAU ``+Q`` is north-dominant, while pyuvdata's declared east-X/north-Y pair requires `XX - YY = -Q`; `M(linear)=I2` instead binds the north-first sky axis to the east-labelled X row. The finding remains **OPEN** because WP-5 has not yet implemented the selected basis permutation, regenerated and explained affected pins/artifacts, or passed its acceptance gate. | 7J discovery; WP-4 ruling complete 2026-08-08; WP-5 implementation pending |
