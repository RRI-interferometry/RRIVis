...
| SCI-006 | OPEN | RadioSim's local Stokes ``Q`` sign disagreed with `pyuvsim`/`pyradiosky` for the same east-X BeamFITS convention and mount. WP-4 ruled the old `M(linear)=I2` binding non-normative. The local WP-5 candidate now owns `P=[[0,1],[1,0]]` in `core/polarization_basis.py`, derives `C=M_native R(chi)` and `H=M_output M_native^H`, produces `XX-YY=-Q`, removes the crossval Q compensation, and passes local solver/output/crossval checks. The finding remains **OPEN** until the controlled six-cell fingerprint regeneration and separate read-only acceptance review complete; no remote evidence has yet been collected. | 7J discovery; WP-4 ruling complete; WP-5 local candidate, acceptance pending 2026-08-08 |
| SCI-007 | OPEN | After SCI-006 removes the basis-axis compensation, direct `Q+iU` comparison against `pyuvsim` refits the residual frame rotation to `+0.0579914273` degrees with relative linear residual `0.00205205064` and ratio modulus `1.00008302` (`output/crossvalidation/2026-08-08-pyuvsim-1.4.0.json`). The explicit pyradiosky V-sign mapping agrees to `4.070e-11` relative. The earlier artifact's `-0.0576` degree fit is retained as historical pre-correction evidence. The independent `0.200` degree apparent-equatorial probe and `0.04`-`0.06` degree CIRS-frame probe remain unreconciled, so SCI-007 stays open for WP-6. | 7J discovery; refitted after WP-5 candidate 2026-08-08 |
