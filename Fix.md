...
| DOC-004 | DONE | README claims 15+ configs while two exist — closed 2026-08-02 (Tier 8 whole-tier acceptance, 8F): `README.md`/`examples/README.md` both state "Four shipped YAML samples" and name all four, and `test_stated_shipped_configuration_count_matches_the_filesystem`/`test_every_named_shipped_configuration_exists` derive and check the count against `ls configs/*.yaml` rather than asserting it | 8 |
| DOC-005 | DONE | README/backend documentation contradicts live backend behavior — closed 2026-08-02 (Tier 8 whole-tier acceptance, 8F): the `README.md` backend/performance section is re-verified claim-by-claim against `output/benchmarks/reference/`; `examples/README.md`'s stale "Numba" sentence is replaced; the four accelerator-named `pyproject.toml` extras (`gpu`, `gpu-cuda`, `gpu-rocm`, `tpu`) and the `"gpu"` keyword are removed, replaced by a single honest `jax` extra; every accelerator/speed sentence in tracked prose and in `src/**/*.py` docstrings either cites `output/benchmarks/reference/` or names `PERF-001`, enforced by two scans (`test_no_accelerator_claim_in_tracked_prose_lacks_a_citation`, `..._in_the_package_lacks_a_citation`) | 8 |
| DOC-006 | DONE | `project.md` is stale and still describes RRIVis — closed 2026-08-02 (Tier 8 whole-tier acceptance, 8F): the file stays untracked and gitignored (`git log --all -- project.md` empty; it has never shipped), `.gitignore:125-129` now explains why (superseded by `CLAUDE.md`/`README.md`/`docs/`, retained locally only, not to be restored as a second architecture surface), and the tracked surface carries no RRIVis-era claim | 8 |
| DOC-007 | DONE | `AGENTS.md` describes an absent Hugging Face app — closed 2026-08-02 (Tier 8 whole-tier acceptance, 8F): the sentence is removed, `huggingface_space/` is confirmed not restored, and no tracked file mentions `huggingface`/`gradio`/a Space, enforced by the naming scan | 8 |
| DOC-008 | DONE | No tracked CI and no real integration/performance suites — closed 2026-08-02 (Tier 8 whole-tier acceptance, 8F): `.github/workflows/ci.yml` is tracked, badge-linked, and demonstrably green on all eight jobs at HEAD `ce7f525` (run `30749117742`); `tests/integration/` now includes a real CLI-to-artifact subprocess test (`test_cli_end_to_end.py`) alongside the pre-existing hybrid/Jones end-to-end suites; `tests/performance/` (10 tests) asserts record honesty per the Tier 6 methodology; the example script and notebook execute inside the `quality` job; docstring doctests execute (41 passed) via a dedicated `pixi run doctest` task and CI step; the docs build is `-W --keep-going` by default and reports zero warnings in a clean worktree | 8 |
| CI-001 | DONE | Closed 2026-08-08 by the Post-Tier-8 WP-2/WP-3 acceptance (`docs/development/ci001_adjudication.md`). The evidence path is complete: a committed comparator classifies harvested runs and forced-experiment variants; machine fingerprints include libc, runner image, NumPy/OpenBLAS runtime dispatch, CPU affinity, and cache topology; main CI and the non-gating nightly sampler restore and retain 30-day characterization artifacts and print fingerprint diffs; passing and failing pins retain observed-digest manifests. Forced run `31255085487` at remote SHA `34d9a9e` reproduced both classes on one Intel Xeon Platinum 8573C runner. V1 (NumPy AVX-512 + OpenBLAS `SkylakeX`) reproduced the historical five failure digests exactly; V2 (effective NumPy AVX2 + `SkylakeX`) left only the circular and heterogeneous cubes divergent; V3 (NumPy AVX-512 + OpenBLAS `Haswell`) left only `config.yaml` divergent; V4 (effective AVX2 + `Haswell`) restored every pin to the original class byte-for-byte. The three raw-cube deltas against CI run `31093915574`'s accepted references were `max|dV|=7.105427357601002e-15` (27,460/363,600 elements, max relative `1.815848028416042e-14`), `1.5742275254395323e-21` (540/1,080 elements, max relative `1.0` only because the reference is zero), and `3.469446951953614e-18` (9/48 elements, max relative `6.26764878758987e-17`). All satisfy the full Tier 6 Section 13.5 predicate (`rtol=1e-12`, `atol=1e-12*max(1,max|reference|)`), so the scientifically reproducible second class is accepted; no tolerance was weakened and no arbitrary output was admitted. The discriminator is the effective NumPy/OpenBLAS runtime dispatch state, explaining why CPU model alone failed. Observation-set membership remains primary; the full Section 13.5 verdict is mechanized on a novel digest; only a third legitimate class or a non-byte-stable class converts this cell to a reference-cube tolerance gate with advisory digests. The former block on CI-green claims is lifted. | post-Tier-8 WP-2/WP-3 |
| API-001 | DONE | Closed 2026-08-11 by WP-1 independent acceptance at exact candidate `87bdaf24d796b645c1d9cfe8634e9a023c68ae84`. Implementation `3ac628217b164eef2c885b00786edcde1d707419` broadcasts all four Stokes inputs under the selected array namespace. NumPy, JAX-CPU, and Dask analytic mixed-rank cases pass; incompatible shapes still raise `ValueError`; six backend/dtype cells prove equal-shaped float32/float64 inputs byte-identical to the reconstructed pre-fix arithmetic. Both production callers remain matched-shape. | post-Tier-8 WP-1, accepted 2026-08-11 |
| API-002 | DONE | Closed 2026-08-11 by WP-1 independent acceptance at exact candidate `87bdaf24d796b645c1d9cfe8634e9a023c68ae84`. Implementation `62e53e6bc6f0ae2e98f4dbc1869e89b67e149995` escapes caller text in all four console helpers and configures `RichHandler(markup=False)`. The real offline-preflight message retains its bracketed model list, and a complete helper/logger caller audit found no intentional Rich-markup dependency. | post-Tier-8 WP-1, accepted 2026-08-11 |
...
### 2026-08-02 Tier 8 whole-tier acceptance (Tier 8F)

**VERDICT: ACCEPTED.** Tier 8 (Documentation and release reconciliation) is
accepted as a whole. This is also the closing gate of the entire eight-tier
remediation programme this document defines (Tiers 0–8). Independent
re-review, run against `Tier8ReleasePlan.md` §18's sixteen whole-tier
criteria and §19's `DOC` closure evidence map, from current source and fresh
command execution — not from any slice's own claims. Scope: the indivisible
Tier 8 range `95a937e` (Tier 7 whole-tier acceptance) → `ce7f525` (30
commits: the design gate and its three corrections, 8A–8E each with their
implementation, corrections, and acceptance commits, including 8C's
rejection→repair→re-acceptance cycle). Branch `main`, HEAD `ce7f525`, already
pushed (`git status`: up to date with `origin/main`). No branch created, no
push performed by this review; the only files touched are `Fix.md` and
`Tier8ReleasePlan.md`.

**§18 whole-tier criteria — all sixteen re-proved.**

1. **Example executes, CI runs both.** `pixi run python examples/scripts/simple_simulation.py --help` and the bare invocation both exit 0 locally (prints both fingerprints, writes no artifact); `.github/workflows/ci.yml`'s `quality` job runs both (`ci.yml:181-186`) and is green on run `30749117742`.
2. **Flag parity, proved by test.** `tests/unit/test_tier8_release_acceptance.py::test_examples_readme_documents_no_flag_the_script_does_not_define` / `..._every_flag_the_script_defines` pass inside the full suite; `examples/README.md` names exactly `--config`/`--backend`/`--progress`, matching the script's three `add_argument` calls.
3. **`-W --keep-going` by default, zero warnings, clean detached worktree.** `docs/Makefile`'s `SPHINXOPTS` default includes `-W --keep-going` (`ci.yml`'s own comment confirms it passes no override). Reproduced in a fresh `git worktree add --detach ce7f525`: `make -C docs clean html` → "build succeeded"; the only "warning" substring in the raw log is `suppress_warnings=[]` inside a MyST debug repr, not a real warning. Worktree removed after.
4. **`docs/api/` coverage.** `docs/index.rst`'s toctree includes `api/{simulator,result,algorithms,core,sky,jones,backends,io,observability,visualization,utils}`; `automodule` grep confirms entries for `core.sky` (and its nine sub-subpackages), `radiosim.simulator`/`.base`/`.rime`, `core.result`, `radiosim.utils` (and five submodules), `radiosim.visualization`, `core.observability`, and all nine previously-uncovered `core/` modules (`visibility_healpix`, `contraction`, `hybrid`, `phase_center`, `polarization_basis`, `precision`, `receptor`, `solver_partition`, `time_grid`).
5. **Doctests execute and pass, named task + CI step.** `pixi run doctest` (`python -m pytest --doctest-modules src/radiosim -p no:cacheprovider`): **41 passed** in `default` (py311) and **41 passed** in `py312`, reproduced directly in both environments. `ci.yml`'s `quality` job runs the same task and is green.
6. **Symbol/path resolution, proved by test.** `test_every_documented_relative_path_exists` and `test_every_documented_radiosim_symbol_is_importable` pass inside the full suite (both environments, full run below).
7. **No removed name outside the historical allow-list, proved by test.** `test_no_removed_name_is_documented_as_live` passes; direct greps confirm `GeometricDelayJones` (zero hits in `docs/`), `generate_baselines` (zero repository hits), `numba`/`NumbaBackend`, and `calculation_type` appear only in explanatory/historical context in `README.md`/`AGENTS.md`/`CLAUDE.md`/`examples/README.md`.
8. **No `RRIVis` outside the historical allow-list, proved by test.** `test_no_tracked_file_carries_the_pre_rename_project_name` passes; `AGENTS.md` read in full carries no RRIVis reference (fixed at 8E).
9. **Config counts/names derived from `configs/`, proved by test.** `test_stated_shipped_configuration_count_matches_the_filesystem` / `test_every_named_shipped_configuration_exists` pass; `README.md`/`examples/README.md` both say "Four" and name all four; `ls configs/*.yaml` independently confirms four.
10. **Every accelerator/speed sentence cites a benchmark record or `PERF-001`, proved by test.** `test_no_accelerator_claim_in_tracked_prose_lacks_a_citation` and `..._in_the_package_lacks_a_citation` pass; `pyproject.toml` carries no `gpu`/`tpu`/`cuda`/`rocm` extra or keyword (independently confirmed via `tomllib`: `optional-dependencies` keys are exactly `jax, dask, ms, dev, docs, all`).
11. **Shared git-scoped lister, regression-proved.** `tests/support/repo_scan.py` backs all 20 converted call sites in the 11 files of `Tier8ReleasePlan.md` §12's table; `tests/unit/test_utils/test_repo_scan.py` (7 tests, including `test_a_gitignored_checkpoint_under_src_breaks_no_repository_scan`) passes — independently re-run in isolation (`pixi run test -- tests/integration/ tests/unit/test_utils/test_repo_scan.py -n 0`: **68 passed**).
12. **`SKY-002` closure re-verified.** `get_required_services` on `configs/realistic_foreground_example.yaml` returns both services; running it offline (`radiosim validate` plus a direct offline `Simulator.setup()`, per the 8D record) prints the network-dependent pre-flight branch; `test_every_network_implicated_loader_declares_a_service` passes. Register: `SKY-002` is `DONE`, tier `8` (already flipped at 8D, unchanged here).
13. **CLI-to-artifact integration test.** `tests/integration/test_cli_end_to_end.py` (5 tests) re-run directly: passes, including the failure path; invokes the installed console script as a real subprocess and reads the on-disk manifest/HDF5/summary JSON back.
14. **`CI-001` evidence path live.** `_record_machine_fingerprint()` fires unconditionally; confirmed directly in CI job logs for run `30749117742`'s `linux-64 / Python 3.11` cell (job `91499853644`): the "Surface the machine fingerprint (CI-001)" step printed two per-worker records (`cpu model: 'AMD EPYC 7763 64-Core Processor'` — a **fourth** distinct CPU model now on file for this cell, alongside the three named in the original filing), and "Upload the characterization record (CI-001)" uploaded `characterization-linux-64-py311` as a build artifact. Numeric-delta reporting on pin failure (`max|dV|`, relative delta, differing-element count) is unchanged source since 8A/8D and was not exercised this run because nothing failed.
15. **Release metadata agreement, changelog, no accelerator extra.** `pyproject.toml:7`, `pixi.toml:3`, `src/radiosim/__about__.py:3`, `docs/conf.py:15-16` all read `0.3.0`; `pixi run radiosim --version` prints `radiosim, version 0.3.0`. `docs/changelog.rst` has `[0.3.0] - 2026-08-02` covering Tiers 1–8 (including the Tier 8-native `SKY-002` `network_service`→`network_services` rename) with a "Known limitations" section naming all eight open/roadmap rows by ID (`PERF-001`, `SCI-004`, `SCI-005`, `SCI-006`, `SCI-007`, `CI-001`, `API-001`, `API-002`), read in full and confirmed complete. `test_no_accelerator_named_extra_is_published` passes.
16. **CI polled to completion on the acceptance SHA, all eight jobs by run ID.** Run `30749117742` at HEAD `ce7f525`, polled via `gh run view` from `in_progress` to `completed`/`success`. Per-job (all `completed`/`success`): `NumPy/JAX-CPU backend parity` (91499853565), `Lint, metadata, types, and docs` (91499853574), `linux-64 / Python 3.12` (91499853599), `osx-arm64 / Python 3.11` (91499853604), `osx-64 / Python 3.12` (91499853609), `osx-arm64 / Python 3.12` (91499853632), `osx-64 / Python 3.11` (91499853637), `linux-64 / Python 3.11` (91499853644). **All eight green — the first Tier 8 CI evidence, since nothing was pushed mid-tier.**

**`CI-001` disposition.** No red was observed on this run — `linux-64 / Python 3.11` passed, consistent with the filed ~38% recurrence rate (this is one of the majority-passing runs). No Q3 adjudication is triggered (that conditional applies only to a fingerprint-observation red, and none occurred), so no digest is appended and the row is not narrowed. The run does add new, honest evidence to the structural gap `CI-001` names: an *accepted-digest* sample's machine fingerprint is now on record for a CPU model (`AMD EPYC 7763 64-Core Processor`) not previously seen on this cell, with dispatched-feature list `AVX,AVX2,F16C,FMA3,MMX,POPCNT,SSE,SSE2,SSE3,SSE41,SSE42,SSSE3` — narrower than either Intel run's reported set, same shape as the other AMD sample, consistent with (but not proof of) the module's already-falsified feature-dispatch theory carrying no discriminating power either way. Per this review's instructions and the standing "open rows stay as they are" rule, `CI-001`'s register row text and status are **not edited** by this acceptance; this paragraph is the disclosure of the new observation. `CI-001` **remains `OPEN`**.

**Empirical gate, exact counts, both gating environments, fresh where the plan asks for it.**

- `pixi run test -- -m "not slow" -n 0`, `default` (py311): **5376 passed, 1 skipped, 10 deselected, 27 warnings** (551.70s). Identical command, `py312`: **5376 passed, 1 skipped, 10 deselected, 41 warnings** (597.51s). Matches the plan's stated expectation (5,376/1/10, 27/41) exactly, in both environments, `-n 0`.
- `pixi run test` (full suite, no marker filter, `default`): **5386 passed, 1 skipped, 27 warnings** (194.64s) — exactly 5376 + the 10 previously-deselected `slow` tests.
- `pixi run --environment crossval test -- -m crossval`: **4 passed** (47.68s) — `test_installed_pyuvsim_is_the_recorded_reference_version`, `test_the_committed_artifact_describes_this_comparison`, `test_unpolarized_point_sources_match_pyuvsim`, `test_polarized_sources_with_jones_p_match_pyuvsim_up_to_the_basis_swap`.
- `pixi run doctest`: **41 passed**, both `default` and `py312`.
- `pixi run bench`: **10 passed** (13.43s) — all `tests/performance/test_backend_benchmarks.py` record-honesty assertions.
- `pixi run lint`: clean ("All checks passed!").
- `pixi run check-format`: clean, **378 files** already formatted.
- `pixi run typecheck`: strict Pyright ceiling satisfied, **2583 <= 4600**.
- `pixi lock --check`: "Lock-file was already up-to-date".
- `pixi install --locked` for `default`, `py312`, and `crossval`: all three installed cleanly with no re-solve.
- Strict docs build, fresh detached worktree at `ce7f525` (`git worktree add --detach`, `.pixi` not shared): `make -C docs clean html` → **build succeeded, zero warnings**. Worktree removed after.
- All four `configs/*.yaml` (`config.yaml`, `hybrid_sky_example.yaml`, `realistic_foreground_example.yaml`, `receptor_circular_example.yaml`): `radiosim validate` clean on every one.
- Laziness gate: `pixi run test -- -k lazy -n 0`: **20 passed, 1 skipped, 5366 deselected** — matches the standing gate exactly.
- `examples/scripts/simple_simulation.py`: `--help` and the bare invocation both exit 0; the bare run prints `Scientific fingerprint: ca3b3a2e...` and `Provenance fingerprint: c60d737b...`, writes no artifact.
- `examples/notebooks/01_basic_usage.ipynb`: `pixi run -- jupyter nbconvert --to notebook --execute --stdout ...` exits 0, produces a valid executed notebook document on stdout.
- `pixi run radiosim --version`: `radiosim, version 0.3.0`.
- `tests/integration/` + `tests/unit/test_utils/test_repo_scan.py` run in isolation: **68 passed** (97.18s), including the CLI-to-artifact test and the `.ipynb_checkpoints` regression proof.
- `git status --porcelain`: clean before, during (worktrees only, removed with `git worktree remove --force`), and after this review.

**§17's fifteen implementation items, walked item-by-item, all landed.**

1. `examples/scripts/simple_simulation.py` rewritten against public API only — 8B, re-verified above. 2. Executed in CI — `ci.yml:181-186`, green. 3. README/Sphinx low-level baseline examples fixed — `generate_baselines` has zero hits; Sphinx builds at zero warnings. 4. Removed Jones class names replaced — 19-name `__all__`, scan 1 enforced. 5. Backend status updated from measured Tier 6 results — `README.md:346-380` re-verified against `output/benchmarks/reference/`; changelog's "Known limitations" names `PERF-001`. 6. The 15+ config claim replaced — "Four shipped YAML samples", derived and tested. 7. `project.md` disposition decided — left untracked, `.gitignore` explains why (Section 15.1). 8. `huggingface_space/` disposition decided — not restored; `AGENTS.md` sentence removed. 9. Real CLI-to-output integration tests added — `tests/integration/test_cli_end_to_end.py`. 10. Real performance tests using Tier 6 methodology — `tests/performance/test_backend_benchmarks.py` (pre-existing, re-verified: 10/10 pass). 11. Sphinx built with warnings as errors — `docs/Makefile`'s default, zero warnings. 12. Every documentation code example executed or validated — doctests (state 1, 41 passed) plus the symbol/path scans (state 2) for `.rst`/`.md` prose blocks. 13. Changelog and migration guide updated for breaking changes — `[0.3.0]` section covers Tiers 1–8, `docs/migration_guide.md` carries the `SKY-002` rename entry. 14. Final repository-wide RRIVis/nonexistent-symbol/unsupported-claim/stale-count sweep — the eight `tests/unit/test_tier8_release_acceptance.py` scans. 15. Git-scoped file listing extracted and applied to every `rglob` site — `tests/support/repo_scan.py`, 20 sites in 11 files, regression-proved.

**Inter-record consistency, 8A–8E (including 8C's rejection cycle) and the design corrections.** Re-read every slice's acceptance record in full (this session) rather than re-deriving from summaries. No two records disagree on a fact: the fingerprint-instrumentation move (Section 14 items 2/3, 8D→8A) is stated identically in the design correction, 8A's own record, and every later slice's references to it; `CI-001`'s "nothing to measure pre-push" conclusion at 8D is consistent with 8E's "no live GitHub Actions run exists" disclosure and with this review being the first to observe a real run; the `SKY-002` register flip is asserted once (at 8D, by the implementer per the plan's own prescription) and never contradicted afterward; the `API-001`/`API-002` filings are each disclosed once, at the review that found them (8B, 8D respectively), and consistently carried into 8E's changelog correction and this review's "open rows stay as they are" list. 8C's rejection→repair→re-acceptance cycle is internally consistent: the first review's two defects (the non-hermetic gitignore-guard test; the duplicated `utils` facade content) are each independently re-verified fixed by this review's own commands (the hermetic probe-file test passes in both pre-existing-directory and probe-created-directory shapes; every one of `radiosim.utils`'s 14 `__all__` symbols resolves to exactly one anchor site-wide). All bounded plan corrections applied across the tier (the 8A `AGENTS.md` "five"→"six" wording fix, the 8C intersphinx-network-sensitivity ruling and the two additional scan-7 instances, the 8D `docs/migration_guide.md` writable-list grant, the 8E changelog Known-limitations completeness fix) are present in `Tier8ReleasePlan.md` at HEAD and none was left half-applied.

**Unobserved, honestly.** This review did not attempt to name `CI-001`'s discriminator (out of Tier 8's scope throughout the tier, unchanged here); did not exercise GPU/TPU hardware (none exists in this environment); did not install `actionlint` (unavailable, as at 8D — the workflow was instead exercised by actually running it on GitHub Actions, which is strictly stronger evidence than static linting); did not attempt a second CI run to see whether `CI-001` reproduces on a second observation (one run was polled to completion, per the review's mandate — a second run was not requested and would not change any criterion's disposition, since the criterion is "poll to completion and report," not "poll until a specific outcome").

**Register.** `DOC-001` through `DOC-008` flip `DOCS` → `DONE` (§5, above), each with closure evidence re-proved from current source and fresh command execution, not carried over from any slice's unverified claim. `SKY-002` stays `DONE` (flipped at 8D, unchanged). `PERF-001`, `SCI-004`, `SCI-005` stay `ROADMAP`; `SCI-006`, `SCI-007`, `CI-001`, `API-001`, `API-002` stay `OPEN` — all eight are named with register IDs in the `[0.3.0]` changelog's "Known limitations" section, so none is silently absorbed by this tier's completion. **Tier 8 is ACCEPTED as a whole. The eight-tier RadioSim remediation programme (Tiers 0–8) is ACCEPTED in full.**

### 2026-08-02 Remediation program completion

The programme `Fix.md` opened is complete. Final state of every register row,
by final disposition:

**DONE (48 rows):** `REL-001`, `REL-002`, `CFG-001`, `CFG-002`, `CFG-003`,
`INS-001`, `INS-002`, `INS-003`, `BEAM-001`, `BEAM-002`, `BEAM-003`,
`OBS-001`, `OBS-002`, `OUT-001`, `OUT-002`, `OUT-003`, `OUT-004`, `OUT-005`,
`OUT-006`, `POL-001`, `POL-002`, `RUN-001`, `RUN-002`, `RUN-003`, `RUN-004`
(narrowed to correctness-parity scope, its accelerator-performance remainder
filed as `PERF-001`), `SKY-001`, `SKY-002`, `RUN-005`, `RUN-006`, `SCI-001`,
`SCI-002`, `SCI-003`, `DOC-001`, `DOC-002`, `DOC-003`, `DOC-004`, `DOC-005`,
`DOC-006`, `DOC-007`, `DOC-008` — every question this programme originally
asked about configuration, instrument/baseline resolution, beam and
observability wiring, the result/output model, receptor and polarization
feeds, the hybrid runtime and backend correctness, Jones-term physics, and
documentation/release truth is closed, each behind a test or a re-verified
citation.

**ROADMAP (3 rows), disclosed, not owned by any tier's exit criteria:**
`PERF-001` (accelerator performance — hardware-gated, no GPU/TPU in any
environment this programme ran in), `SCI-004` (a spherical-harmonic/m-mode
solver — a second forward-model algorithm, not a value on a removed field),
`SCI-005` (beam physics beyond the accepted scalar-`E` subset — polarized/
cross-polar beams, squint, blockage, Zernike aberrations, the Ruze
error-beam decomposition — each scoped and cited in
`docs/development/beam_physics_scope.md`).

**OPEN (5 rows), disclosed, non-blocking, named in the `[0.3.0]` changelog:**
`SCI-006` (local Stokes-`Q` sign versus `pyuvsim`/`pyradiosky` — a
characterization, not yet a ruling on which convention is intended),
`SCI-007` (a residual `-0.0576°` frame-rotation gap, unreconciled against two
independent probes of the right order of magnitude), `CI-001` (an
intermittent, unidentified-discriminator second digest class on
`linux-64-py311`, ~38% observed recurrence, root-cause naming explicitly out
of this programme's scope, its evidence path now live and unconditionally
recording — this review's own CI run added a fourth CPU-model data point
without resolving the discriminator), `API-001` (`stokes_to_coherency`
broadcast ergonomics, no solver path affected), `API-002` (`print_warning`
markup-swallowing, a caller-message display defect, no solver path
affected).

**What this programme did not do**, stated once more so it is not
mistakenly read into anything above: it made no GPU/TPU/accelerator
performance claim; it implemented no m-mode or spherical-harmonic solver; it
made no advanced polarized-beam-physics claim; it did not resolve `SCI-006`
or `SCI-007` in either direction; it did not name `CI-001`'s discriminator;
it did not tag, release, or publish anything — no `git tag`, no GitHub
release, no PyPI upload, no `readthedocs` configuration change, and this
acceptance itself performs no push. The version bump to `0.3.0` is tracked
metadata and changelog content only.

Nine tiers (0 through 8, each independently designed and accepted, with
per-slice independent adversarial review throughout, including one
documented rejection→repair→re-acceptance cycle at Tier 8C and one at Tier 6J)
took the project from a state where most Jones terms were identity stubs,
configuration inputs were only partially honored, backend claims were
unmeasured, and public documentation described a differently-named,
differently-scoped, partly-fictional predecessor package — to a state where
every exported Jones term implements real, cited, tested physics or is
explicitly filed as roadmap; every configuration surface is centrally
resolved and exhaustively validated; every backend-correctness claim is
tested to a stated tolerance and every backend-performance claim is either
cited to a committed benchmark record or explicitly disclosed as
undemonstrated; and every public document, example, and API surface is
either executed, scanned, cited, or absent — never merely asserted. The
programme is complete.

### 2026-08-11 WP-1 independent acceptance — API-001 and API-002

The bounded post-Tier-8 fixes were independently accepted at exact candidate
`87bdaf24d796b645c1d9cfe8634e9a023c68ae84`. API-001 implementation commit
`3ac628217b164eef2c885b00786edcde1d707419` adds the adopted broadcasting
step; API-002 implementation commit
`62e53e6bc6f0ae2e98f4dbc1869e89b67e149995` makes caller-supplied console and
logger text literal. Test commit `87bdaf2` adds the previously missing
NumPy/JAX/Dask analytic and unchanged-path proof for API-001.

The independent reviewer reproduced both old failures from the pre-fix
implementations rather than inferring a red-first history from the co-committed
fix tests. The accepted focused command passed **32 tests**: 24 polarization
tests, seven logging tests, and the shipped offline-preflight test. Mixed-rank
broadcasts matched an independently assembled coherency matrix on NumPy,
JAX-CPU, and Dask; incompatible shapes still raised; and float32/float64
equal-shape results were byte-identical to reconstructed pre-fix arithmetic in
all six backend/dtype cells. The old Rich path swallowed the bracketed model
list, while the accepted path preserved it. All 12 production helper call
sites and repository logger calls were audited with no intentional Rich-markup
dependency.

Fresh acceptance gates also passed: Ruff lint, the 386-file format check,
Pyright's checked ceiling (`2584 <= 4600`), 41 doctests, `git diff --check`,
and `git show --check 87bdaf2`. The existing `[Unreleased]` changelog entries
